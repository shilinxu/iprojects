...
public:
	...
	// Wait for access for this object. Return if successful in
	// timeout specified. Zero timeout checks and returns w/o delay
	// Lock and Wait are equivalent.
...
/*	Mutex - only one thread at a time can own the mutex. If the thread trys
	to lock mutex that already owned by another thread it will be block
	( for the timeout ) until owning thread unlock the mutex.
	Mutexes are typically used to protect shared resources that can be
	accessed only by one thread of execution.
*/
...
/*	More than one thread can lock a semaphore class at the same time.
	If the thread attempts to lock the semaphore and the maximum number
	of locks has been reached it will block ( for the timeout ) until one
	of the owners unlocks the semaphore.
	Semaphores are typically used to protect shared resources that can be
	accessed by limited number threads of execution at the same time.
*/
...
/*	A WEvent object exists in two states - signaled and non-signaled.
	When signaled threads attempting to lock this object do not block -
	otherwise the threads are blocked until the object changes to the
	signaled state.
*/
...
public:
	...
	// Pulse(signal) the event, releasing waiting threads and then
	// return to the non-signaled state.
	void Pulse();
...
/*	Single lock object contains only one sync object so it may be locked
	by one thread at a time. If another thread attemps to lock an object
	that has already been locked by another thread, that requesting thread
	will block until the owning thread unlocks the object.
...
{
public:
	// Constructs the single lock object from synctonization object,
	// that cannot be NULL. If initialLock is true the object is locked
	// then it constructed.
	iSingleLock( iSyncObject* syncObject, bool initalLock = false );

	// Destruct this single lock. If it has a lock on a syncronization
	// object it will be released
	~iSingleLock();
	...
	// Unlocks the sync object for specified count which must
	// be greather than zero. Returns false if not successful.
...
/*	Multi-lock object contains more the one sync object. When a threads
	requests a lock, the first available one is returned. If none are
	available, the task blocks - for the given timeout. Alternatively
	a thread can requests all locks at once.
...
public:
	...
	// Constructs a multi-lock from the specified syncronizaion objects
	// that cannot be NULL. If initialLock is true the object is locked
	...
	// Destruct this multilock. If it has a lock on a sync object it will
	// be released.
	~iMultiLock();

	// Locks the syncronization object. Returns (FAILED) if lock failed.
	// Returns (OBJECT) to (OBJECT+num-1) an index into the array of
	// objects, indicating which object satisfied the wait. ( if more then
	// one - smallest index returned )
	Result Lock( uint32 timeout = INFINITE, bool all = false );

	// Unlocks the syncronization object. Always return true.
	bool Unlock();

	// Unlocks the syncronizaion object for specified count which
	// must be greather than zero. Returns false if not successful
	bool Unlock( uint32 count );

	// Test the sync object at the specified index for the locked
	// state. Returns true if it is locked.
