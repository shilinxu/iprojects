...
CButtonST::~CButtonST()
{
	...
	m_BitmapRgn1.DeleteObject();
	m_BitmapRgn2.DeleteObject();
	m_BitmapRgn3.DeleteObject();
	m_BitmapRgn4.DeleteObject();
...
void CButtonST::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		...
		// Destroy bitmaps
		if (m_csBitmaps.hBitmap)	::DeleteObject(m_csBitmaps.hBitmap);


		// Destroy mask bitmaps
		if (m_csBitmaps.hMask)	::DeleteObject(m_csBitmaps.hMask);
	} // if
	...
	if(pMsg->message == WM_LBUTTONDOWN)
	{
		...
		{			
			if(m_b3StateBtn)
			{
				if (m_bMouseOnButton)
				{
					if(!m_BitmapRgn3.PtInRegion(pMsg->pt))
					{
						return TRUE;
					}
				}
				else
				{
					if(m_bSelect)
					{
						if(!m_BitmapRgn2.PtInRegion(pMsg->pt))
						{
							return TRUE;
						}
					}
					...
					{
						if(!m_BitmapRgn1.PtInRegion(pMsg->pt))
						{
							return TRUE;
						}
					}
			...
			else if(m_bCheckBtn)
			{
				if (m_bMouseOnButton)
				{
					if(m_bSelect)
					{
						if(!m_BitmapRgn4.PtInRegion(pMsg->pt))
						{
							return TRUE;
						}
					}
...
void CButtonST::CancelHover()
{
	...
	if (m_bIsFlat)
	{
		if (m_bMouseOnButton)
		{
			GetParent()->SendMessage(WM_MOUSE_OUT, m_nID, 0);
			m_bMouseOnButton = FALSE;			
			Invalidate();
		} // if
	} // if
...
void CButtonST::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (wndUnderMouse && wndUnderMouse->m_hWnd == m_hWnd && wndActive)
	{
		if (m_bRgnBtn)
		{
			if(m_b3StateBtn)
			{
				if (!m_bMouseOnButton)
				{
					if(m_bSelect)
					{
						if(!m_BitmapRgn2.PtInRegion(point))
						{
							CancelHover();
							return;
						}
					...
					{
						if(!m_BitmapRgn1.PtInRegion(point))
						{
							CancelHover();
							return;
						}
				...
				{
					if(!m_BitmapRgn3.PtInRegion(point))
					{
						CancelHover();
						return;
					}
			...
			else if(m_bCheckBtn)
			{
				...
				else
				{
					if(m_bSelect)
					{
						if(!m_BitmapRgn4.PtInRegion(point))
						{
							CancelHover();
							return;
						}
		...
		if (!m_bMouseOnButton)
		{
			GetParent()->SendMessage(WM_MOUSE_OVER, m_nID, 0);
...
{	
	...
	else if(m_bActionBtn)
	{	
		EnableWindow(FALSE);
		SetTimer(ACTION_TIMER, 200, NULL);
	}
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	CRect	rImage;
	if(m_b3StateBtn)
		PrepareImageRect(bHasTitle, rpItem, rpCaption, bIsPressed, m_csBitmaps.dwWidth/3, m_csBitmaps.dwHeight, &rImage);
	else if(m_bCheckBtn)
		PrepareImageRect(bHasTitle, rpItem, rpCaption, bIsPressed, m_csBitmaps.dwWidth/4, m_csBitmaps.dwHeight, &rImage);
	else if(m_bActionBtn)
		PrepareImageRect(bHasTitle, rpItem, rpCaption, bIsPressed, m_csBitmaps.dwWidth/4, m_csBitmaps.dwHeight, &rImage);
	else
		PrepareImageRect(bHasTitle, rpItem, rpCaption, bIsPressed, m_csBitmaps.dwWidth/2, m_csBitmaps.dwHeight, &rImage);

	hdcBmpMem = ::CreateCompatibleDC(pDC->m_hDC);
	hbmOldBmp = (HBITMAP)::SelectObject(hdcBmpMem, m_csBitmaps.hBitmap);
	hdcMem = ::CreateCompatibleDC(NULL);
	hbmT = (HBITMAP)::SelectObject(hdcMem, m_csBitmaps.hMask);

	if(m_b3StateBtn)
	{
		int	nWidth = m_csBitmaps.dwWidth/3;
		// Select the bitmap to use		
		if (m_bMouseOnButton)
		{	
			::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcMem, nWidth*2, 0, SRCAND);
			::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcBmpMem, nWidth*2, 0, SRCPAINT);
		}	
		else if(m_bSelect)
		{
			::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcMem, nWidth, 0, SRCAND);
			::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcBmpMem, nWidth, 0, SRCPAINT);
		}
		else
		{
			::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcMem, 0, 0, SRCAND);
			::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcBmpMem, 0, 0, SRCPAINT);		
		}
	}
	else if(m_bCheckBtn)
	{
		int	nWidth = m_csBitmaps.dwWidth/4;
		if(m_bSelect)
		{
			if (m_bMouseOnButton)
			{	
				::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcMem, nWidth*3, 0, SRCAND);
				::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcBmpMem, nWidth*3, 0, SRCPAINT);
			}	
			else if(m_bSelect)
			{
				::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcMem, nWidth, 0, SRCAND);
	...
	else if(m_bActionBtn)
	{
		...
		if(m_nActionCount > -1)
		{
			...
			{
				::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcMem, nWidth*(m_nActionCount+2), 0, SRCAND);
				::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcBmpMem, nWidth*(m_nActionCount+2), 0, SRCPAINT);
			}
		}
		else
		{
	...
	{
		int	nWidth = m_csBitmaps.dwWidth/2;
		...
		else
		{
			...
			pDC->FillSolidRect(CRect(21,24,35,30), m_clrColor);
			pDC->FillSolidRect(CRect(16,25,40,29), m_clrColor);
			pDC->FillSolidRect(CRect(14,26,42,28), m_clrColor);						
		}
	}
	else if(m_bWidthBtn)
	{
		...
		{	
			...
			pDC->DrawText(m_strWidth, CRect(11,18,31,30), DT_CENTER);
		...
		else if(m_bMouseOnButton)
		{
			...
			pDC->SetBkMode(TRANSPARENT);
			pDC->SetTextColor(RGB(70,70,70));
			pDC->DrawText(m_strWidth, CRect(10,17,30,29), DT_CENTER);
		}
		else
		{
			::BitBlt(pDC->m_hDC, rImage.left, rImage.top, nWidth, m_csBitmaps.dwHeight, hdcMem, 0, 0, SRCAND);
			...
			pDC->DrawText(m_strWidth, CRect(10,18,30,30), DT_CENTER);
		}		
	}
	else
	{
...
//
DWORD CButtonST::SetBitmaps(int nBitmap, COLORREF crTransColor)
{
	...
	return SetBitmaps(hBitmap, crTransColor);
} // End of SetBitmaps
...
//			Failed creating mask bitmap.
//
DWORD CButtonST::SetBitmaps(HBITMAP hBitmap, COLORREF crTransColor)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
	...
	if (hBitmap)
	{
		m_csBitmaps.hBitmap = hBitmap;
		m_csBitmaps.crTransparent = crTransColor;
		// Get bitmap size
		...
		m_csBitmaps.dwWidth = (DWORD)csBitmapSize.bmWidth;
		m_csBitmaps.dwHeight = (DWORD)csBitmapSize.bmHeight;
		
		if(m_bRgnBtn)
			MakeRgn();

		// Create mask for bitmap In
		m_csBitmaps.hMask = CreateBitmapMask(hBitmap, m_csBitmaps.dwWidth, m_csBitmaps.dwHeight, crTransColor);
		if (m_csBitmaps.hMask == NULL)
		{
			FreeResources();
...
void CButtonST::MakeRgn()
{
	...
	SelectObject(dcImage.m_hDC, m_csBitmaps.hBitmap);

	CRect rcWnd;
	GetWindowRect (rcWnd);
	
	m_BitmapRgn1.DeleteObject();
	m_BitmapRgn1.CreateRectRgn (rcWnd.left, rcWnd.top, rcWnd.right, rcWnd.bottom);
	m_BitmapRgn2.DeleteObject();
	m_BitmapRgn2.CreateRectRgn (rcWnd.left, rcWnd.top, rcWnd.right, rcWnd.bottom);
	m_BitmapRgn3.DeleteObject();
	m_BitmapRgn3.CreateRectRgn (rcWnd.left, rcWnd.top, rcWnd.right, rcWnd.bottom);		
	m_BitmapRgn4.DeleteObject();
	m_BitmapRgn4.CreateRectRgn (rcWnd.left, rcWnd.top, rcWnd.right, rcWnd.bottom);
	...
	m_BitmapRgn1.CombineRgn(&m_BitmapRgn1, &rgnClient, RGN_XOR);

	rgnClient.DeleteObject();
	rgnClient.CreateRectRgn (rcClient.left, rcClient.top, rcClient.right, rcClient.bottom);
	m_BitmapRgn2.CombineRgn(&m_BitmapRgn2, &rgnClient, RGN_XOR);

	rgnClient.DeleteObject();
	rgnClient.CreateRectRgn (rcClient.left, rcClient.top, rcClient.right, rcClient.bottom);
	m_BitmapRgn3.CombineRgn(&m_BitmapRgn3, &rgnClient, RGN_XOR);

	rgnClient.DeleteObject();
	rgnClient.CreateRectRgn (rcClient.left, rcClient.top, rcClient.right, rcClient.bottom);
	m_BitmapRgn4.CombineRgn(&m_BitmapRgn4, &rgnClient, RGN_XOR);

	int row_start, height, width;	
	height = m_csBitmaps.dwHeight;
	
	if(m_b3StateBtn)
		width = m_csBitmaps.dwWidth/3;
	else if(m_bCheckBtn)
		width = m_csBitmaps.dwWidth/4;
	else
		width = m_csBitmaps.dwWidth/2;
	...
	{			
		...
		for (x=0; x<width; x++)
		{
			if (dcImage.GetPixel(x, y) == m_csBitmaps.crTransparent)
			{
				...
				else
				{
					...
					m_BitmapRgn1.CombineRgn (&m_BitmapRgn1, &rgnAdd, RGN_OR);
					row_start = x+1;
				}
			}
		}
	...
	for (y=0; y<height; y++)
	{
		...
		for (x=0; x<width; x++)
		{
			if (dcImage.GetPixel(nX, y) == m_csBitmaps.crTransparent)
			{
				if (row_start == x) row_start ++;
				else
				{
					...
					rgnAdd.CreateRectRgn (rcClient.left+row_start,
						rcClient.top+y, rcClient.left+x, rcClient.top+y+1);
					m_BitmapRgn2.CombineRgn(&m_BitmapRgn2, &rgnAdd, RGN_OR);
					row_start = x+1;
				}
			}
			...
			for (x=0; x<width; x++)
			{
				if (dcImage.GetPixel(nX, y) == m_csBitmaps.crTransparent)
				{
					...
					else
					{
						...
						rgnAdd.CreateRectRgn (rcClient.left+row_start,
							rcClient.top+y, rcClient.left+x, rcClient.top+y+1);
						m_BitmapRgn3.CombineRgn(&m_BitmapRgn3, &rgnAdd, RGN_OR);
						row_start = x+1;
					}
				}
			...
			for (x=0; x<width; x++)
			{
				if (dcImage.GetPixel(nX, y) == m_csBitmaps.crTransparent)
				{
					...
					else
					{
						...
						rgnAdd.CreateRectRgn (rcClient.left+row_start,
							rcClient.top+y, rcClient.left+x, rcClient.top+y+1);
						m_BitmapRgn4.CombineRgn(&m_BitmapRgn4, &rgnAdd, RGN_OR);
						row_start = x+1;
					}
				}
...
void CButtonST::SetControlID(int nID)
{
	m_nID = nID;
}


void CButtonST::SetRgnBtn(BOOL bRgnBtn)
{
	m_bRgnBtn = bRgnBtn;
}


void CButtonST::Set3StateButton(BOOL b3State)
{
	m_b3StateBtn = b3State;
}


void CButtonST::SetActionButton(BOOL bAction)
{
	m_bActionBtn = bAction;
}


void CButtonST::SetCheckButton(BOOL bCheckBtn)
{
	m_bCheckBtn = bCheckBtn;
}

void CButtonST::SetColorButton(BOOL bColorBtn)
{
	m_bColorBtn = bColorBtn;
}

void CButtonST::SetButtonColor(COLORREF color)
{
	m_clrColor = color;
	Invalidate();
}

void CButtonST::SetWidthButton(BOOL bWidthBtn)
{
	m_bWidthBtn = bWidthBtn;	
}

void CButtonST::SetButtonWidth(int nWidth)
{
	m_strWidth.Format("x%d", nWidth);
	Invalidate();
}


void CButtonST::SetSelect(BOOL bSelect)
{
...
void CButtonST::SizeToContent()
{
	...
	else if (m_csBitmaps.hBitmap)
	{
		m_ptImageOrg.x = 0;
		m_ptImageOrg.y = 0;
	    SetWindowPos(NULL, -1, -1, m_csBitmaps.dwWidth/2, m_csBitmaps.dwHeight,
						SWP_NOMOVE | SWP_NOZORDER | SWP_NOREDRAW | SWP_NOACTIVATE);
	} // if
...
void CButtonST::OnTimer(UINT nIDEvent) 
{
	...
	if(nIDEvent == ACTION_TIMER)
	{
		...
		else
		{
			m_nActionCount = -1;
			Invalidate();			
			KillTimer(ACTION_TIMER);
			EnableWindow(TRUE);
			GetParent()->SendMessage(WM_SNAPSHOT, 0, 0);			
		}
	}
